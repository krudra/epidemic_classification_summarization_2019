...
def getprocedure(DIC,METAMAP):
        dic = eval(DIC)
        A = []
	tag = ['topp','sosy','phsf','bdsu','blor','bpoc','dora','acty']
...
def summarize_prevention(ifname,orgname,ofname,SL):
	...
	for l in fp:
		...
		if tag=='prevention':
			count+=1
			temp = getprocedure(wl[2],METAMAP)
			...
			T[index] = temp
			TW[index] = [tid,text_org,temp,int(wl[3]),conf]
			index+=1
	fp.close()
	...
	tweet_cur_window = {}
        for i in range(0,L,1):
        	temp = TW[i]
                tweet_cur_window[temp[0].strip(' \t\n\r')] = [temp[1],temp[3],set(temp[2]),temp[4]]   ### Text, Length, Content words, Confidence ###
...
def optimize(tweet,weight,ofname,L):
	...
	for  k,v in tweet.iteritems():
		...
		tweet_word[tweet_index] = [v[1],set_of_words,v[0],v[3]]  #Length of tweet, set of content words present in the tweet, tweet itself
		tweet_index+=1
...
def main():
	try:
		...
		print(e)
		sys.exit(0)
	summarize_prevention(ifname,orgname,ofname,int(SL))
	print('Koustav Done')
